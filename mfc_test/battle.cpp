...
POINT cardDect[6][2];//对于每张卡，选取两个像素点进行判断卡的属性
	...
	for (int i = 1; i <= 5; i++)
	{
		...
		for (int j = 0; j <= 1; j++)
		{
			...
			else
			{
				...
				else if (cl.B >= 100 && cl.R <= 50 && cl.G <= 50)//被克制,这个有挺大概率是测不出来的，待修正
				{
					card[i].isWeak = -1;
...
void onBattle(int info[4][20],int eventFlag)//处理整个副本的流程
{
